...
public class CodeAttribute extends AttributeInfo implements Opcode
{
	...
	private static void shiftIndex(CodeIterator ci, int lessThan, int delta) throws BadBytecode
	{
		...
		if (opcode < Opcode.ILOAD)
			return;
		else if (opcode < Opcode.IASTORE)
		{
			if (opcode < Opcode.ILOAD_0)
				// iload, lload, fload, dload, aload
				CodeAttribute.shiftIndex8(ci, index, opcode, lessThan, delta);
			else if (opcode < Opcode.IALOAD)
				// iload_0, ..., aload_3
				CodeAttribute.shiftIndex0(ci, index, opcode, lessThan, delta, Opcode.ILOAD_0, Opcode.ILOAD);
			else if (opcode < Opcode.ISTORE)
				return;
			else if (opcode < Opcode.ISTORE_0)
				// istore, lstore, ...
				CodeAttribute.shiftIndex8(ci, index, opcode, lessThan, delta);
			else
				// istore_0, ..., astore_3
				CodeAttribute.shiftIndex0(ci, index, opcode, lessThan, delta, Opcode.ISTORE_0, Opcode.ISTORE);
	...
	private static void shiftIndex0(CodeIterator ci, int index, int opcode, int lessThan, int delta, int opcode_i_0, int opcode_i) throws BadBytecode
	{
		...
		else
		{
			...
			{
				int pos = ci.insertExGap(3);
				ci.writeByte(Opcode.WIDE, pos - 1);
	...
	{
		out.writeShort(this.name); // attribute_name_index
		out.writeInt(this.length() - 6); // attribute_length
		out.writeShort(this.maxStack); // max_stack
		out.writeShort(this.maxLocals); // max_locals
		out.writeInt(this.info.length); // code_length
		out.write(this.info); // code
		this.exceptions.write(out);
		out.writeShort(this.attributes.size()); // attributes_count
		AttributeInfo.writeAll(this.attributes, out); // attributes
	}
}
