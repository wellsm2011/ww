...
public class MapMaker extends Tracer
{
	 ...
	 * main2(args); return; } for (int i = 0; i < args.length; i++)
	 * main1(args[i]); } public static void main1(String className) throws
	 * Exception { ClassPool cp = ClassPool.getDefault(); //javassist.CtClass cc
	 * = cp.get(className); javassist.CtClass cc = cp.makeClass(new
	 * java.io.FileInputStream(className)); System.out.println(className);
	 * ClassFile cf = cc.getClassFile(); java.util.List minfos =
	 * cf.getMethods(); for (int i = 0; i < minfos.size(); i++) { MethodInfo
	 * minfo = (MethodInfo)minfos.get(i); CodeAttribute ca =
	 * minfo.getCodeAttribute(); if (ca != null) ca.setAttribute(make(cp,
	 * minfo)); } cc.writeFile("tmp"); } public static void main2(String[] args)
	 * throws Exception { ClassPool cp = ClassPool.getDefault();
	 * //javassist.CtClass cc = cp.get(args[1]); javassist.CtClass cc =
	 * cp.makeClass(new java.io.FileInputStream(args[1])); MethodInfo minfo; if
	 * (args[2].equals("_init_")) minfo =
	 ...
	 * javassist.JvstTest4.testJIRA195().
	 */
	...
	public StackMapTable toStackMap(TypedBlock[] blocks)
	{
		...
		if (prev.incoming > 0)
		{ // the first instruction is a branch target.
			writer.sameFrame(0);
			offsetDelta--;
		}
