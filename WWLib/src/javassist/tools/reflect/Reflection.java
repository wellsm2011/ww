...
public class Reflection implements Translator
{
	...
	{
		return name.startsWith(ClassMetaobject.methodPrefix) || name.equals(Reflection.classobjectAccessor) || name.equals(Reflection.metaobjectSetter) || name.equals(Reflection.metaobjectGetter)
				|| name.startsWith(Reflection.readPrefix) || name.startsWith(Reflection.writePrefix);
	}
	...
	public boolean makeReflective(CtClass clazz, CtClass metaobject, CtClass metaclass) throws CannotCompileException, CannotReflectException, NotFoundException
	{
		...
		if (clazz.subclassOf(this.classPool.get(Reflection.classMetaobjectClassName)))
			throw new CannotReflectException("Cannot reflect a subclass of ClassMetaobject: " + clazz.getName());

		if (clazz.subclassOf(this.classPool.get(Reflection.metaobjectClassName)))
	...
	private boolean modifyClassfile(CtClass clazz, CtClass metaobject, CtClass metaclass) throws CannotCompileException, NotFoundException
	{
		...
		if (addMeta)
		{
			f = new CtField(this.classPool.get("javassist.tools.reflect.Metaobject"), Reflection.metaobjectField, clazz);
			f.setModifiers(Modifier.PROTECTED);
			clazz.addField(f, CtField.Initializer.byNewWithParams(metaobject));

			clazz.addMethod(CtNewMethod.getter(Reflection.metaobjectGetter, f));
			clazz.addMethod(CtNewMethod.setter(Reflection.metaobjectSetter, f));
		}

		f = new CtField(this.classPool.get("javassist.tools.reflect.ClassMetaobject"), Reflection.classobjectField, clazz);
		f.setModifiers(Modifier.PRIVATE | Modifier.STATIC);
		clazz.addField(f, CtField.Initializer.byNew(metaclass, new String[]
		{ clazz.getName() }));

		clazz.addMethod(CtNewMethod.getter(Reflection.classobjectAccessor, f));
		return true;
	}
	...
	private void processFields(CtClass clazz) throws CannotCompileException, NotFoundException
	{
		...
		for (int i = 0; i < fs.length; ++i)
		{
			...
			if ((mod & Modifier.PUBLIC) != 0 && (mod & Modifier.FINAL) == 0)
			{
				...
				CtMethod wmethod = CtNewMethod.wrapped(ftype, Reflection.readPrefix + name, this.readParam, null, this.trapRead, ConstParameter.string(name), clazz);
				wmethod.setModifiers(mod);
				clazz.addMethod(wmethod);
				CtClass[] writeParam = new CtClass[2];
				writeParam[0] = this.classPool.get("java.lang.Object");
				writeParam[1] = ftype;
				wmethod = CtNewMethod.wrapped(CtClass.voidType, Reflection.writePrefix + name, writeParam, null, this.trapWrite, ConstParameter.string(name), clazz);
				wmethod.setModifiers(mod);
				clazz.addMethod(wmethod);
			}
		}
	...
	private void registerReflectiveClass(CtClass clazz)
	{
		...
		for (int i = 0; i < fs.length; ++i)
		{
			...
			{
				String name = f.getName();
				this.converter.replaceFieldRead(f, clazz, Reflection.readPrefix + name);
				this.converter.replaceFieldWrite(f, clazz, Reflection.writePrefix + name);
			}
		}
	}
