 ...
 * Especially, there might be security flaws in this server. Please use this
 * with YOUR OWN RISK.
 */
public class Webserver
{
	...
	public void doReply(InputStream in, OutputStream out, String cmd) throws IOException, BadHttpRequest
	{
		...
		if (filename.endsWith(".class"))
			fileType = Webserver.typeClass;
		else if (filename.endsWith(".html") || filename.endsWith(".htm"))
			fileType = Webserver.typeHtml;
		else if (filename.endsWith(".gif"))
			fileType = Webserver.typeGif;
		else if (filename.endsWith(".jpg"))
			fileType = Webserver.typeJpeg;
		else
			fileType = Webserver.typeText; // or textUnknown

		len = filename.length();
		if (fileType == Webserver.typeClass && this.letUsersSendClassfile(out, filename, len))
			return;
		...
		if (fileType == Webserver.typeClass)
		{
			...
			if (fin != null)
			{
				...
				this.sendHeader(out, classfile.length, Webserver.typeClass);
				out.write(classfile);
				fin.close();
				return;
			}
	...
	private void sendHeader(OutputStream out, long dataLength, int filetype) throws IOException
	{
		...
		else if (filetype == Webserver.typeHtml)
			out.write("Content-Type: text/html".getBytes());
		else if (filetype == Webserver.typeGif)
			out.write("Content-Type: image/gif".getBytes());
		else if (filetype == Webserver.typeJpeg)
			out.write("Content-Type: image/jpg".getBytes());
		else if (filetype == Webserver.typeText)
