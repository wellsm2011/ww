...
public class Javac
{
	 ...
	 *
	 * @param src
	 *            a single statement or a block. If null, this method produces a
	 *            body returning zero or null.
	 */
	...
	private CtField compileField(FieldDecl fd) throws CompileError, CannotCompileException
	{
		...
		f = new CtFieldWithInit(this.gen.resolver.lookupClass(d), d.getVariable().get(), this.gen.getThisClass());
	...
	{
		this.gen.setProceedHandler(h, Javac.proceedName);
	}
	...
	public int recordReturnType(CtClass type, boolean useResultVar) throws CompileError
	{
		...
		return this.gen.recordReturnType(type, "$r", useResultVar ? Javac.resultVarName : null, this.stable);
	}
