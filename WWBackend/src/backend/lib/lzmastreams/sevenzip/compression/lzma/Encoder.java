package backend.lib.lzmastreams.sevenzip.compression.lzma;

import java.io.IOException;

import backend.lib.lzmastreams.sevenzip.ICodeProgress;
import backend.lib.lzmastreams.sevenzip.compression.rangecoder.BitTreeEncoder;
...
public class Encoder
{
	class LenEncoder
	{
		...
		public void Encode(backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder rangeEncoder, int symbol, int posState) throws IOException
		{
		...
		{
			backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.InitBitModels(this._choice);
		...
		{
			int a0 = backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice0(this._choice[0]);
			int a1 = backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._choice[0]);
			int b0 = a1 + backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice0(this._choice[1]);
			int b1 = a1 + backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._choice[1]);
			int i = 0;
	...
	class LiteralEncoder
	{
		class Encoder2
		{
			...
			public void Encode(backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder rangeEncoder, byte symbol) throws IOException
			{
				int context = 1;
				for (int i = 7; i >= 0; i--)
				{
			...
			public void EncodeMatched(backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder rangeEncoder, byte matchByte, byte symbol) throws IOException
			{
				int context = 1;
				boolean same = true;
				for (int i = 7; i >= 0; i--)
			...
			public int GetPrice(boolean matchMode, byte matchByte, byte symbol)
			{
				...
				if (matchMode)
					for (; i >= 0; i--)
					{
						int matchBit = matchByte >> i & 1;
						int bit = symbol >> i & 1;
						price += backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice(this.m_Encoders[(1 + matchBit << 8) + context], bit);
						context = context << 1 | bit;
						if (matchBit != bit)
						{
							i--;
				...
				{
					int bit = symbol >> i & 1;
					price += backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice(this.m_Encoders[context], bit);
					context = context << 1 | bit;
				}
				return price;
			}
			...
			public void Init()
			{
				backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.InitBitModels(this.m_Encoders);
			}
		}
	...
	backend.lib.lzmastreams.sevenzip.compression.lz.BinTree			_matchFinder			= null;
	backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder	_rangeEncoder			= new backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder();
	...
	void Create()
	{
		...
		{
			backend.lib.lzmastreams.sevenzip.compression.lz.BinTree bt = new backend.lib.lzmastreams.sevenzip.compression.lz.BinTree();
			int numHashBytes = 4;
	...
	void FillDistancesPrices()
	{
		...
		for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++)
		{
			...
			for (posSlot = Base.kEndPosModelIndex; posSlot < this._distTableSize; posSlot++)
				this._posSlotPrices[st + posSlot] += (posSlot >> 1) - 1 - Base.kNumAlignBits << backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.kNumBitPriceShiftBits;
	...
	int GetOptimum(int position) throws IOException
	{
		...
		this._optimum[1].Price = backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice0(this._isMatch[(this._state << Base.kNumPosStatesBitsMax) + posState])
		...
		int matchPrice = backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._isMatch[(this._state << Base.kNumPosStatesBitsMax) + posState]);
		int repMatchPrice = matchPrice + backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._isRep[this._state]);
		...
		int normalMatchPrice = matchPrice + backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice0(this._isRep[this._state]);
		...
		while (true)
		{
			...
			int curAnd1Price = curPrice + backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice0(this._isMatch[(state << Base.kNumPosStatesBitsMax) + posState])
			...
			matchPrice = curPrice + backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._isMatch[(state << Base.kNumPosStatesBitsMax) + posState]);
			repMatchPrice = matchPrice + backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._isRep[state]);
			...
			if (!nextIsChar && matchByte != currentByte)
			{
				...
				if (lenTest2 >= 2)
				{
					...
					int nextRepMatchPrice = curAnd1Price + backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext])
							+ backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._isRep[state2]);
					{
				...
				if (lenTest < numAvailableBytesFull)
				{
					...
					if (lenTest2 >= 2)
					{
						...
						int curAndLenCharPrice = repMatchPrice
								...
								+ backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice0(this._isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext])
						...
						int nextMatchPrice = curAndLenCharPrice
								+ backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
						int nextRepMatchPrice = nextMatchPrice + backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._isRep[state2]);
			...
			if (newLen >= startLen)
			{
				normalMatchPrice = matchPrice + backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice0(this._isRep[state]);
	...
	int GetPureRepPrice(int repIndex, int state, int posState)
	{
		...
		if (repIndex == 0)
		{
			price = backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice0(this._isRepG0[state]);
			price += backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
		} else
		{
			price = backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._isRepG0[state]);
			if (repIndex == 1)
				price += backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice0(this._isRepG1[state]);
			else
			{
				price += backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice1(this._isRepG1[state]);
				price += backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice(this._isRepG2[state], repIndex - 2);
			}
		}
		return price;
	}
	...
	{
		return backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice0(this._isRepG0[state])
				+ backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.GetPrice0(this._isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
	}
	...
	void Init()
	{
		...
		backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.InitBitModels(this._isMatch);
		backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.InitBitModels(this._isRep0Long);
		backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.InitBitModels(this._isRep);
		backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.InitBitModels(this._isRepG0);
		backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.InitBitModels(this._isRepG1);
		backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.InitBitModels(this._isRepG2);
		backend.lib.lzmastreams.sevenzip.compression.rangecoder.Encoder.InitBitModels(this._posEncoders);
